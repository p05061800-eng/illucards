...
- **После оформления** успешный ответ `POST /api/order/create` очищает **корзину в браузере** перед переходом в Telegram (чтобы не оставались те же позиции локально).
- **После принятия заказа админом** (статус вроде «Принят» / `confirmed`) сервер дополнительно **сбрасывает синхронизированную корзину** пользователя в Telegram (см. `app/api/order/update/route.ts` и связанные вызовы) — заказ живёт в **личном кабинете**, а не «висит» в корзине.
...
- **Бонусные баллы** начисляются один раз на заказ при переходе в подходящий статус (в т.ч. **«Принят»**, а также отправка/доставка — см. `app/lib/bonusProgram.ts` и `bonus_awarded` в `app/lib/orderTypes.ts`), с защитой от повторного начисления.
