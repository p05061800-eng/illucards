...
- **После оформления заказа на сайте** корзина в браузере и синхронизированная корзина в боте **не очищаются сразу** — пользователь может доредактировать корзину, пока не отметит оплату.
- **Корзина очищается** (сайт + Redis/синк с ботом) только после статуса **`paid`** («чек оплаты отправил»): в боте кнопка **«💳 Чек оплаты отправил»** → `POST /api/order/update` с `status: "paid"` (см. `app/api/order/update/route.ts`).
